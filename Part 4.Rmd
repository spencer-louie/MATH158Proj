...
lot.data <- data.frame(sqft_lot=lot.grid)
sort(kc_data_trial$sqft_lot)
...
```{r}
## ALL OF THE CODE FROM HERE TO LINES DOES NOT WORK AT THE MOMENT
kc.lor1 <- loess(price ~ sqft_lot, span=.2, data=kc_data_trial)
...
```
```{r}
kc.lor1.pred.fit <-kc.lor1.pred$fit
```
...
lines(kc_data_trial$sqft_lot, kc.lor1.pred$fit , lwd=2, col="blue")
