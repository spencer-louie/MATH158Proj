...
```{r}
kc_data_trial <- kc_data_trial %>% mutate(grade2 = grade^2)
kc4.lm.full3<- lm(price ~ as.factor(grade), data=kc_data_trial)
kc4.lm.red3<- lm(price ~ grade + grade2, data=kc_data_trial)
anova(kc4.lm.red3,kc4.lm.full3)
ggplot(data=kc_data, aes(x=factor(grade), y=lprice)) + stat_summary(fun.y="mean", geom="point")
```
