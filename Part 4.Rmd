...
anova(kc4.lm.full1,kc4.lm.red1)
anova(kc4.lm.full2,kc4.lm.red2)

```
```{r}
kc4.lm.full3<- lm(price ~ as.factor(bedbath), data=kc_data_trial)
kc4.lm.red3<- lm(price ~ 0 + bedbath^2, data=kc_data_trial)
anova(kc4.lm.red3,kc4.lm.full3)
plot(fitted(lm(log(price)~bedbath, data=kc_data)), resid(lm(log(price) ~ bedbath, data=kc_data)))
```
