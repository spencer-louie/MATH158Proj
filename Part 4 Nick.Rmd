...
```{r}
avPlots(lm(log(price) ~ sqft_living + log(sqft_lot) + floors + waterfront + view + log(condition) + grade + bedbath, data=kc_data_train), terms=~sqft_living)
```
...
```{r}
require(gam)
kc.gam <- gam(log(price) ~ log(sqft_living) + log(sqft_lot) + as.integer(floors) + waterfront + factor(view) + s(condition, df=6) + s(grade,df=6) + s(bedbath, df=6), data=kc_data.train)
summary(kc.gam)
AIC(kc_mlr.lm)
AIC(kc.gam)
par(mfrow=c(1,3))
